...
| `billing/stripe` | Thin, storage-free wrapper over the Stripe API for **subscription billing**: `CheckoutURL` (hosted Checkout Session — cards/3DS/promo codes), `PortalURL` (Customer Portal), `EnsureCustomer`, `SessionSubscription`/`CustomerSubscription` (sync on the checkout/portal return without waiting on the webhook), and `VerifyWebhook`. Stripe owns renewals + discounts (coupons); you map your plan keys to price ids and persist subscription state. | early |
| `env` | Tiny zero-dependency `.env` loader: `Load(path)` reads `KEY=VALUE` into the process environment without overriding already-set vars (so the real env / systemd always wins). No config struct, no coercion — pair it with your own config over `os.Getenv`. | early |
